...
int advui_max_num_nodes()
{
    ...
    try {
        ...
        // If even the boot-time File handle cannot be constructed, select the
        // smaller database rather than rebooting or committing another ~22 KB.
        LOG_WARN("advui: radio-mode read OOM; using the low-memory node cap");
        cached = 32;
        return cached;
    }
