...
so the photo assets stay current with the UI:
    ...
    python3 scripts/mockup.py --config docs/img/mockup.json \
        --frame hil-artifacts/private/visual/frames/a19.png --out /tmp/hero.png

Release media is normally generated by ``scripts/demo_media.py``; this lower-level
tool remains useful for inspecting one private framebuffer capture locally.
